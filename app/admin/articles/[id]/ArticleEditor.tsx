      ...
      <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {article.statut === "PUBLISHED" && (
            <SocialShareModal article={{ id: article.id, titre: form.titre, chapo: form.chapo, slug: article.slug, tags: form.tags.split(",").map((t) => t.trim()).filter(Boolean), imageUrl: form.imageUrl || null, categorie: article.categorie }} variant="editor" />
          )}
        </div>
      </div>
